...
TR=0.333;
seconds=60;
time=round((0:1*PhantomSize-1));
Movingphantom=zeros([size(SLPhantom) numel(time)]);
omeg=20;
livermotionrange=0.08*PhantomSize;
...
% implay(Movingphantom,6)
%% Reconstruct moving data from acquisition
Acquiredkspace_AP=zeros(size(Movingphantom));
Acquiredkspace_RL=zeros(size(Movingphantom));

for frame=1:numel(time)
    kspaceduringAcq=fftshift(fft(fftshift(fft(Movingphantom(:,:,frame),[],1),1),[],2),2);
    Acquiredkspace_AP(:,frame,frame)=kspaceduringAcq(:,frame);
    Acquiredkspace_RL(frame,:,frame)=kspaceduringAcq(frame,:);
end
TotalAcq_AP=sum(Acquiredkspace_AP,3);
TotalAcq_RL=sum(Acquiredkspace_RL,3);
...
% 
% subplot(1,3,2)
% imagesc(real(ifft(ifftshift(ifft(ifftshift(TotalAcq_AP,1),[],1),2),[],2)));
% daspect([1 1 1]);title('Readout AP')
% 
% subplot(1,3,3)
% imagesc(real(ifft(ifftshift(ifft(ifftshift(TotalAcq_RL,1),[],1),2),[],2)));
% daspect([1 1 1]);title('Readout RL')
%% Motion correction
...
% Shiftoperator=zeros(size(Movingphantom));
% Shiftoperator=exp(pi*1i*kx.*permute(repmat(LiverMotionVector.',[1 numel(time) numel(time)]),[2 3 1]) + pi*1i*ky.*permute(repmat(LiverMotionVector.',[1 numel(time) numel(time)]),[2 3 1]));
% Shiftoperator=exp(pi*1i*kx.*LiverMotionVector.' + pi*1i*ky.*LiverMotionVector.'*0);

Correctedkspace_AP=zeros(size(Movingphantom));
Correctedkspace_RL=zeros(size(Movingphantom));

for frame=1:numel(time)
    kspaceduringAcq=fftshift(fft(fftshift(fft(Movingphantom(:,:,frame),[],1),1),[],2),2);
    Correctedlines=kspaceduringAcq.*exp(pi*1i*kx.*LiverMotionVector(frame) + pi*1i*ky.*LiverMotionVector(frame)*0);
    Correctedkspace_AP(:,frame,frame)=Correctedlines(:,frame);
    Correctedkspace_RL(frame,:,frame)=Correctedlines(frame,:);
    
end

TotalCorrected_AP=sum(Correctedkspace_AP,3);
TotalCorrected_RL=sum(Correctedkspace_RL,3);
...
subplot(2,3,5)
imagesc(real(ifft(ifftshift(ifft(ifftshift(TotalCorrected_AP,1),[],1),2),[],2)));
daspect([1 1 1]);title('Readout AP')

subplot(2,3,6)
imagesc(real(ifft(ifftshift(ifft(ifftshift(TotalCorrected_RL,1),[],1),2),[],2)));
daspect([1 1 1]);title('Readout RL')
